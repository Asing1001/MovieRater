...
function getCacheCharacteristics(loaderOptions, source, filename, transformMode) {
    var _fileExtensionRegex_exec;
    const { isServer, pagesDir } = loaderOptions;
    const isPageFile = filename.startsWith(pagesDir);
    const isNextDist = nextDistPath.test(filename);
    const hasModuleExports = source.indexOf('module.exports') !== -1;
    const fileNameOrExt = transformMode === 'default' ? ((_fileExtensionRegex_exec = fileExtensionRegex.exec(filename)) == null ? void 0 : _fileExtensionRegex_exec[1]) || 'unknown' : filename;
    return {
        isServer,
        isPageFile,
        isNextDist,
 ...
 */ async function getFreshConfig(ctx, cacheCharacteristics, loaderOptions, target, filename, inputSourceMap) {
    ...
    if (loaderOptions.transformMode === 'standalone') {
        ...
        options.sourceMaps = loaderOptions.sourceMaps === undefined ? ctx.sourceMap : loaderOptions.sourceMaps;
        options.plugins = [
 ...
 */ function getCacheKey(cacheCharacteristics) {
    const { isServer, isPageFile, isNextDist, hasModuleExports, fileNameOrExt } = cacheCharacteristics;
...
async function getConfig(ctx, { source, target, loaderOptions, filename, inputSourceMap }) {
    const cacheCharacteristics = getCacheCharacteristics(loaderOptions, source, filename, loaderOptions.transformMode);
    ...
    const freshConfig = await getFreshConfig(ctx, cacheCharacteristics, loaderOptions, target, filename, inputSourceMap);
