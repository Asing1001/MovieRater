...
export function getYahooMovieInfo(yahooId: number) {
  const defer = Q.defer();
  const yahooMovieUrl =
    'https://movies.yahoo.com.tw/movieinfo_main.html/id=' + yahooId;
  var req = request(
    { url: yahooMovieUrl, followRedirect: false },
    (error, res, body) => {
      if (error) {
      ...
      const movieInfo: YahooMovie = {
        ...
        types: Array.from($movieInfoDiv.find('.level_name_box a')).map((a) =>
          $(a).text().trim()
        ),
