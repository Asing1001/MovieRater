...
class TheaterList extends React.PureComponent<any, any> {
  ...
  setTheatersWithDistance = (theaters) => {
    getClientGeoLocation().then(({ latitude, longitude }) => {
      const theatersWithDistance = theaters
        .map((theater: Theater) => {
          const { location } = theater;
          if (location && location.lat && location.lng) {
            const { lat, lng } = location;
            return { ...theater, distance: getDistanceInKM(lng, lat, longitude, latitude) };
          } else {
            return theater; // If lat and lng are missing, return the original theater
          }
        })
    ...
    return (
      <div>
        ...
        {this.state.theaters
          .filter(
            ({ subRegion }) =>
              this.state.selectedSubRegion === defaultSubRegion || subRegion === this.state.selectedSubRegion
          )
