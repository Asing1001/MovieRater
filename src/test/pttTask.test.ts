...
describe('pttCrawler', () => {
    ...
    describe('updatePttArticles', () => {
        it('should get new pttArticles then updateDocument', async function () {
            const pttPage: PttPage = { pageIndex: 99999, url: 'http://ptt.test', articles: [{ title: "test", url: "http://article" }] };
            const stubGetDocument = sandbox.stub(db, 'getDocument').returns({ maxPttIndex: 9999 });
