...
describe('atmovieInTheaterCrawler', () => {
  ...
  describe('updateMoviesSchedules', () => {
    it.skip('should.above(0)', async function () {
      ...
      const stubGetCollections = sandbox
        .stub(Mongo, 'getCollection')
        .returns([{ scheduleUrl: '/1/' }, { scheduleUrl: '/2/' }]);
      const stubGetDocument = sandbox
        .stub(Mongo, 'getDocument')
        .returns({ scheduleDay: 1 });
      const stubCrawlSchedule = sandbox
        .stub(movieSchduleCrawler, 'crawlMovieSchdule')
        .returns([]);
